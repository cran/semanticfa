...
#' @param baseline Optional numeric vector, one value per pool word, giving
...
#'   particular. Default \code{NULL} leaves scores uncentered.
#' @param baseline_sd Optional numeric vector, one value per pool word,
#'   giving the standard deviation of that word's null target cosine. When
#'   supplied together with \code{baseline}, scores are standardized rather
#'   than merely centered. Default \code{NULL}.
...
sfa_nameability <- function(lexmap, loadings, n_candidates = 5L,
                            ...
                            baseline = NULL, baseline_sd = NULL) {
  ...
  picks <- vector("list", k)
  loo <- vector("list", k)
  sc_list <- vector("list", k)
  for (j in seq_len(k)) {
    sc <- as.numeric(lexmap$S %*% coefs[[j]])
    if (!is.null(baseline)) {
      # surprise scoring (experimental v2): standardize each word's cosine
      # against that word's own null-target distribution
      sc <- sc - baseline
      if (!is.null(baseline_sd)) sc <- sc / baseline_sd
    }
    sc_list[[j]] <- sc
...
#' @keywords internal
.lexi_loo_S <- function(L, pole, j, lexmap, gate, baseline = NULL,
                        baseline_sd = NULL) {
  sel <- pole$sel[[j]]
  ...
  b_sub <- if (is.null(baseline)) 0 else baseline[gate$row]
  s_sub <- if (is.null(baseline_sd)) 1 else baseline_sd[gate$row]
  n <- lexmap$n_items
  score_sub <- function(cc) (as.numeric(Ssub %*% cc) - b_sub) / s_sub
  winners <- integer(0)
  for (drop in seq_along(sel)) {
    ...
    winners <- c(winners, which.max(score_sub(cc)))
  }
  cc <- .lexi_coef(sel, pole$w[[j]], lexmap$Q, n)
  winners <- unique(c(winners, which.max(score_sub(cc))))
...
#' @param baseline,baseline_sd Optional per-pool-word null mean and standard
#'   deviation for surprise scoring, passed through to
#'   [sfa_nameability()]. Default \code{NULL} scores raw target cosines.
...
sfa_leximax <- function(x, Phi = NULL, lexmap = NULL,
  ...
  for (s_idx in seq_along(starts)) {
    ...
    for (it in seq_len(max_iter)) {
      nm <- sfa_nameability(lexmap, Lc, baseline = baseline,
  ...
  if (is.null(baseline)) {
    ...
    canonical <- st$labels$label   # surprise scoring is S-space by definition
    agreement <- NA
  }
  labels <- st$labels
