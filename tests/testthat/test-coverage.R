...
test_that("sfa_deletion_gaps reports what a shortened scale gives up", {
  emb <- rbind(.make_cloud(8, .centers[1, ]),
               .make_cloud(8, .centers[2, ]),
               .make_cloud(8, .centers[3, ]))
  full_items <- .new_items("full", emb)
  # a reduced form that drops the third construct's items entirely, so the
  # deletion has a construct-shaped hole for the gap labeler to find
  short_items <- full_items[1:16]

  # screen_items = FALSE keeps both audits on the same region sentences; the
  # item-dependent circularity screen would otherwise drop different rows for
  # each item set and make the two audits incomparable
  args <- list(region = .fake_region(), embed = .fake_embed,
               model = "fake-encoder", cache = FALSE, sense_gate = FALSE,
               trim = 0, n_boot = 20, screen_items = FALSE)
  reference <- do.call(sfa_coverage, c(list(full_items), args))
  # anchor the reduced audit to the reference radius: recalibrating on the
  # shortened item set would move the yardstick with the items
  reduced <- do.call(sfa_coverage, c(list(short_items), args,
                                     list(radius = reference$radius)))

  gaps <- sfa_deletion_gaps(reference, reduced, quotes = 2L, top = 4L)
  expect_false(is.null(gaps))
  # dropping a third of the items cannot increase construct coverage
  expect_lte(reduced$coverage, reference$coverage + 1e-8)
})

test_that("sfa_deletion_gaps rejects non-audit input", {
  expect_error(sfa_deletion_gaps(list(a = 1), list(b = 2)),
               "sfa_coverage")
})
